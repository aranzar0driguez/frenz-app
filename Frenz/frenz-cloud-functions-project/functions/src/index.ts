...
app.post("/send-notification", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const response = await sendNotification(fcmToken, payload)
...
export const sendNotif = functions.https.onRequest(app)

// endpoint: /getUserAppVersion
export const getUserAppVersion = functions.https.onRequest(async (request, response) => {
  try {
...
functions.https.onRequest(async (request, response) => {
  try {
    ...
    const buildQueries = (randomNum: number) => {
      const queries = []
      ...
      if (fetchRomanticUsers) {
        if (userGender === "female") {
          if (userSexuality === "Females") {
            ...
            queries.push(
              {sex: "female", attracted_sex: "Females"},
              {sex: "female", attracted_sex: "Both"}
            )
          } else if (userSexuality === "Males") {
            // Female attracted to males
            queries.push(
              {sex: "male", attracted_sex: "Females"},
              {sex: "male", attracted_sex: "Both"}
            )
          ...
          if (userSexuality === "Females") {
            ...
            queries.push(
              {sex: "female", attracted_sex: "Males"},
              {sex: "female", attracted_sex: "Both"}
            )
          } else if (userSexuality === "Males") {
            // Male attracted to males
            queries.push(
              {sex: "male", attracted_sex: "Males"},
              {sex: "male", attracted_sex: "Both"}
            )
          } else if (userSexuality === "Both") {
        ...
        return appPurposes.map(purpose =>
          db
            .collection("users")
            .where("random_num", "==", randomNum)
            .where("app_utilization_purpose", "==", purpose)
    ...
    const fetchRandomUserSnapshots = async (randomNum: number) => {
